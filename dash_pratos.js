/* =======================================================================
    dash_pratos.js — v10.6-stable-clean
    - LÓGICA CORRIGIDA: KPIs, Top 10 e Gráfico DOW calculados a partir
      de uma única fonte de dados.
    - CORREÇÃO DE ENCODING: Caracteres invisíveis (U+00A0) removidos.
    ======================================================================= */
...
/* ===================== HELPERS ===================== */
// CORREÇÃO: Garantindo que a função $ seja definida corretamente sem caracteres invisíveis.
...
function setupFilterInteractions(){
  ...
  if (!fx.$btnMore) { console.error("Elementos de filtro não encontrados na UI."); return; }
...
function renderMonthChart(data){
  ...
  if (!canvasEl) {
      console.error(`[${APP_VERSION}] Erro Crítico: Canvas 'ch_month' não encontrado.`);
      setChartMessage('box_month', 'Erro de Renderização (Canvas Ausente).');
      return;
  }
...
function renderDowChart(data, mode='TOTAL'){
  ...
  if (!canvasEl) {
    console.error(`[${APP_VERSION}] Erro Crítico: Canvas 'ch_dow' não encontrado.`);
    setChartMessage('box_dow', 'Erro de Renderização (Canvas Ausente).');
    return;
  }
...
async function fetchRowsPaged(startIso, endIso, filters, limit=5000){
  ...
  for(let from=0; from<count; from+=limit){
    ...
    // Selecionamos apenas as colunas necessárias para o cálculo no frontend
...
function calculateMetricsFromRows(rows, daysInPeriod) {
    ...
    // Média diária = total vendido / número de dias no período selecionado.
...
async function fetchMonthChartData(payload){
  // Calcula os 12 meses terminando no mês do período selecionado.
  ...
  // Prepara a sequência de 12 meses e agrega os dados.
...
async function setupImportFeature(){
  ...
  input.addEventListener('change', async (ev)=>{
    ...
    try{
      ...
      alert(`${processed.length} registros importados! Atualizando dashboard…`);
      // Recarrega os filtros após a importação antes de atualizar o dashboard
...
async function loadFilterOptions(){
  try {
    // Tenta buscar opções de filtro pré-calculadas via RPC
...
async function loadAndInitializeFilters() {
    ...
    // Define "Pratos" como categoria padrão exclusiva
...
async function init(){
  try{
    ...
    // Define a data base (último dia com dados)
    try {
    ...
    // Carrega opções de filtro e inicializa os componentes
    await loadAndInitializeFilters();
