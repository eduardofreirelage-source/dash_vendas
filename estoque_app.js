...
if (!window.supabase) {
...
} else if (SUPABASE_URL_ESTOQUE.startsWith('http')) {
    try {
    ...
    console.error("ERRO CRÍTICO: SUPABASE_URL_ESTOQUE inválida ou não configurada!");
}
...
const GenericCRUD = {
    ...
    async save(e, table, editorId, refreshCallback) {
        ...
        try {
            let query;
            if (id) {
                query = supaEstoque.from(table).update(data).eq('id', id);
            } else {
                query = supaEstoque.from(table).insert([data]);
            }
...
const ReceitasModule = {
    ...
    async showRecipeForm(id = null) {
        ...
        if (id) {
            ...
            try {
                const { data: recData, error: recError } = await supaEstoque.from('receitas').select('*').eq('id', id).single();
                ...
                const { data: itemsData, error: itemsError } = await supaEstoque.from('vw_receita_itens_detalhes').select('*').eq('receita_id', id);
    ...
    async saveRecipe(e) {
        ...
        try {
            let savedRecipe;
            if (id) {
                const { data, error } = await supaEstoque.from('receitas').update(recipeData).eq('id', id).select().single();
                if (error) throw error;
                savedRecipe = data;
                await supaEstoque.from('receita_itens').delete().eq('receita_id', id);
            } else {
                const { data, error } = await supaEstoque.from('receitas').insert([recipeData]).select().single();
                if (error) throw error;
                savedRecipe = data;
            }
            ...
            if (itemsError) {
                if (!id) await supaEstoque.from('receitas').delete().eq('id', savedRecipe.id);
                throw itemsError;
            }
...
const PratosModule = {
    ...
    async loadComponentes(pratoId) {
        ...
        try {
            const { data, error } = await supaEstoque.from('vw_prato_componentes_detalhes').select('*').eq('prato_id', pratoId).order('nome_receita', { ascending: true });
            if (error) throw error;
            tbody.innerHTML = '';
            data.forEach(item => {
                const tr = document.createElement('tr');
    ...
    async addComponente() {
        ...
        try {
            const { error } = await supaEstoque.from('prato_componentes').insert([{
                prato_id: pratoId,
    ...
    async handleComponenteTableClick(e) {
        ...
        if (confirm("Tem certeza que deseja remover este componente?")) {
            setStatus("Removendo...");
            try {
                const { error } = await supaEstoque.from('prato_componentes').delete().eq('id', id);
                if (error) throw error;
    ...
    async showImportUI() {
        ...
        try {
            const { data, error } = await supaEstoque.rpc('get_unregistered_dishes');
            if (error) throw error;
    ...
    async confirmImport() {
        ...
        try {
            const { data: existingCats, error: catError } = await supaEstoque.from('categorias').select('id, nome').eq('tipo', 'PRATO');
            ...
            if (catsToCreate.length > 0) {
                ...
                const { data: insertedCats, error: newCatError } = await supaEstoque.from('categorias').insert(catsToCreate).select('id, nome');
            ...
            if (newPratosToInsert.length > 0) {
                ...
                const { error: pratoError } = await supaEstoque.from('pratos').insert(newPratosToInsert);
...
const ProducaoModule = {
    ...
    async calcularPrevisao() {
        ...
        try {
            const { data, error } = await supaEstoque.rpc('calcular_previsao_venda', {
...
const ComprasModule = {
    ...
    async loadSuggestions() {
        ...
        try {
            const { data, error } = await supaEstoque.from('vw_sugestao_compras').select('*');
            if (error) throw error;
